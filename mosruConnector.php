...
class mosruConnector
{
	...
	public function __construct()
	{
		...
		$this->config = $config;
		$this->cookie = tempnam("/tmp", "mosru-cookie-");
	...
	private function login()
	{
		...
		$ch = curl_init();
		$curlOptions = [
			CURLOPT_URL => 'https://oauth20.mos.ru/sps/j_security_check',
			CURLOPT_POST => true,
			CURLOPT_POSTFIELDS => http_build_query([
				'j_username' => urldecode($this->config['login']),
				'j_password' => urldecode($this->config['password']),
				'accessType' => 'email',
			]),
		];
		curl_setopt_array($ch, $this->curlDefaultOptions + $curlOptions);
		...
		if (strpos($this->response, 'SURNAME') === false) {
			return false;
		}
		else {
